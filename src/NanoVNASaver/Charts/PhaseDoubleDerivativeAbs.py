...
class PhaseDoubleDerivativeAbsChart(PhaseChart):
    """Absolute value of phase second derivative (|d²Phase/dFreq²| in °/MHz²)"""
    ...
    def drawValues(self, qp: QPainter):
        ...
        # Calculate absolute value of second derivative in °/MHz²
        if len(self.data) >= 3:
            ...
            # Second derivative, take absolute value
            self.derivedData = []
        ...
        # Determine min/max for scaling (min is 0 for absolute values)
        if self.fixedValues:
            minAngle = self.minDisplayValue
            maxAngle = self.maxDisplayValue
        elif self.derivedData:
            minAngle = 0
            maxAngle = math.ceil(max(self.derivedData))
        elif self.derivedReference:
            minAngle = 0
