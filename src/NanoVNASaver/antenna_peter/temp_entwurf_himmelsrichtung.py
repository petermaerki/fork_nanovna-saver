...
Es gibt Fälle wo es zwei mögliche Servo-Positionen gibt. Es soll jene Position zurückgegeben werden, die näher an der aktuellen Servo-Position liegt.
  '''
def servo_targed_t(servo_t_actual: float = 0.6, heading_actual_deg: int = 90, heading_target_deg: int = 100, servo_min_t: float = 0.4, servo_max_t: float = 1.1, debug: bool = True) -> float:
  if debug:
    print(
  ...
  servo_t_primary_raw = servo_t_actual + (delta_deg(heading_actual_deg, heading_target_deg) / 360)
  servo_t_alt_raw = servo_t_actual + (delta_deg(heading_actual_deg, heading_alt_deg) / 360)
  ...
  if primary_ok and alt_ok:
    if abs(servo_t_primary_raw - servo_t_actual) <= abs(servo_t_alt_raw - servo_t_actual):
      servo_t_choosen = servo_t_primary_raw
      decision = "both ok, primary has shorter travel"
    else:
      servo_t_choosen = servo_t_alt_raw
    ...
    if abs(servo_t_primary - servo_t_actual) <= abs(servo_t_alt - servo_t_actual):
      servo_t_choosen = servo_t_primary
      decision = "both out of range, primary clamps closer"
    else:
      servo_t_choosen = servo_t_alt
  ...
  if debug:
    ...
    print("decision -> {decision}, chosen: {chosen:.3f}".format(
      decision=decision,
      chosen=servo_t_choosen,
    ))
    sys.stdout.flush()
  ...
  if debug:
    ...
    ax.plot([servo_min_t, servo_max_t], [y_min_line, y_max_line], color="black", linewidth=1)
    def cross_lines(t: float, heading: float, color: str, label: str) -> None:
    ...
    ax.scatter([servo_t_choosen], [heading_from_t(servo_t_choosen)], color="red", label="chosen", zorder=5)
    left_span = servo_min_t - 0.05 * span_t
    right_span = servo_max_t + 0.05 * span_t
    ax.axvspan(left_span, servo_min_t, color="lightgray", alpha=0.4, label="out of range")
    ax.axvspan(servo_max_t, right_span, color="lightgray", alpha=0.4)
    ax.axvline(servo_min_t, color="dimgray", linewidth=2, linestyle="--", label="servo min/max")
    ...
    y_candidates = [
      ...
      heading_target_deg,
      heading_alt_deg,
      heading_from_t(servo_t_choosen),
    ]
...
servo_targed_t(servo_t_actual = 1.05,  heading_actual_deg = 0, heading_target_deg = 845, servo_min_t = 0.4, servo_max_t = 1.1)
